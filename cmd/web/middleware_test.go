...
func TestCommonHeaders(t *testing.T) {
  ...
  // commonHeaders *returns* a http.Handler we can call its ServeHTTP() method,
  ...
  // Check that the middleware has correctly set the Referrer-Policy header on
  // the response.
  ...
  // Check that the middleware has correctly set the X-XSS-Protection header on
  // the response.
  expectedValue = "0"
